...
sequenceDiagram
    ...
    participant T as Inbound Messaging Gateway
    ...
    Note over TECH,B: Completion & Billing
    TECH->>DB: Marks Completed (Status: COMPLETED)
    DB-->>AD: Triggers Approval Request
    AD->>M: Approves Invoice
    M->>B: Creates Customer Invoice
    B-->>M: Returns Invoice ID
    M->>DB: Updates Job with Billing Reference
```
...
| `qb_invoice_id` | String | Index | Billing reference ID |
...
| `phone` | String | Incoming SMS lookup key |
...
### B. Inbound SMS Payload
The webhook from the messaging gateway provides the following core data:
```json
{
  "From": "+15551234567",
...
1.  **Find Customer**: Search the billing system for the dealership name.
2.  **Create Invoice**: Use the `job.id` as the reference number.
3.  **Line Items**: Map `service.name` to a billing catalog item.
...
2.  **Safety Lock**: An invoice cannot be sent to the billing system unless the job status is `COMPLETED` and the admin has clicked the `Approve` button in the UI.
...
-   **Environment Variables**: Securely store API keys for the messaging gateway and OpenAI.

### Security
-   **CORS**: Only allow the production dealership domain.
-   **HMAC Validation**: Verify webhook signatures on all inbound SMS webhooks to prevent spoofing.
...
- `POST /api/jobs/:id/approve`: Admin-only endpoint to trigger the billing sync for a completed job.
...
2.  **Phase 2**: Configure the inbound messaging -> Make.com -> OpenAI loop for automated job entry.
3.  **Phase 3**: Build the Technician Mobile web interface for real-time status updates.
4.  **Phase 4**: Finalize the billing OAuth 2.0 flow for secure invoicing.
