...
async function findVisibleErrorText(targetPage) {
  const candidates = [
    '[role="alert"]',
    '.text-red-700',
    '.text-red-200',
    'text=/unable to reach backend|invalid|failed|request|forbidden|restricted|sign in again|insufficient/i',
  ];
...
test('super admin interactive qa', async ({ browser, page }) => {
  ...
  const loginSuperAdmin = async (
    ...
    if (expectFailure) {
      ...
      return findVisibleErrorText(targetPage);
    }
  ...
  const blockedMessage = await findVisibleErrorText(blockedPage);
  ...
  const invalidTokenError = await findVisibleErrorText(invalidTokenPage);
